 ...
 * Files in the jni/libjpeg, jni/libusb, and jni/libuvc folders may have a different license, see the respective files.
*/
...
public:
	void StartObject() {
		beginValue();
		mOutput += "{";
		mStack.push_back({true, true, true});
	}
	...
	void StartArray() {
		beginValue();
		mOutput += "[";
		mStack.push_back({false, true, false});
	}
	...
	void String(const char *value) {
		if (!mStack.empty() && mStack.back().object && mStack.back().expectingKey) {
			writeKey(value);
			return;
		}
		beginValue();
...
private:
	...
	void finishValue() {
		if (!mStack.empty() && mStack.back().object) {
			mStack.back().expectingKey = true;
		}
	}
	...
	void writeNumber(const char *value) {
		beginValue();
		mOutput += value;
		finishValue();
	}
		...
		for (const unsigned char *cursor = reinterpret_cast<const unsigned char *>(value); *cursor; ++cursor) {
			...
			default:
				if (*cursor < 0x20) {
					...
					mOutput += static_cast<char>(*cursor);
				}
				break;
			}
		}
...
static void write(JsonWriter &writer, const char *key, const char *value) {
	writer.String(key);
	writer.String(value);
}

static void write(JsonWriter &writer, const char *key, uint16_t value) {
	writer.String(key);
	writer.Uint(value);
}

static void write(JsonWriter &writer, const char *key, int32_t value) {
	writer.String(key);
	writer.Int(value);
}

static void write(JsonWriter &writer, const char *key, uint32_t value) {
	writer.String(key);
	writer.Uint(value);
}

static void write(JsonWriter &writer, const char *key, int64_t value) {
	writer.String(key);
	writer.Int64(value);
}

static void write(JsonWriter &writer, const char *key, uint64_t value) {
	writer.String(key);
	writer.Uint64(value);
}
...
static void writerFormat(JsonWriter &writer, uvc_format_desc_t *fmt_desc) {
...
static void writerFormatDescriptions(JsonWriter &writer, uvc_streaming_interface_t *stream_if) {
...
//			write(writer, DESC_PRODUCT, desc->product ? desc->product : "UVC Camera");
	...
	writer.EndObject();
	RETURN(strdup(writer.c_str()), char *);
}
