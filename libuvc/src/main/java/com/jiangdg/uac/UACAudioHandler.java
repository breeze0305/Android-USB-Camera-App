...
public class UACAudioHandler extends Handler {
    ...
    public static  class AudioThread extends Thread {
        ...
        public int getSampleRate() {
            synchronized (mSync) {
                if (mUACAudio == null) {
                    if (!waitLocked(TIMES_OUT_MS, "getSampleRate")) {
                        return -1;
                    }
        ...
        public int getChannelCount() {
            synchronized (mSync) {
                if (mUACAudio == null) {
                    if (!waitLocked(TIMES_OUT_MS, "getChannelCount")) {
                        return -1;
                    }
        ...
        public int getBitResolution() {
            synchronized (mSync) {
                if (mUACAudio == null) {
                    if (!waitLocked(TIMES_OUT_MS, "getBitResolution")) {
                        return -1;
                    }
        ...
        public UACAudio.AudioStatus getAudioStatus() {
            synchronized (mSync) {
                if (mUACAudio == null) {
                    if (!waitLocked(TIMES_OUT_MS, "getAudioStatus")) {
                        return UACAudio.AudioStatus.ERROR;
                    }
        ...
        public boolean isRecording() {
            synchronized (mSync) {
                if (mUACAudio == null) {
                    if (!waitLocked(TIMES_OUT_MS, "isRecording")) {
                        return false;
                    }
        ...
        public void run() {
            ...
            try {
                ...
                Log.e(TAG, "UACAudioHandler new failed", e);
            }
            if (uacHandler != null) {
                synchronized (mSync) {
        ...
        public UACAudioHandler getHandler() {
            ...
            synchronized (mSync) {
                if (mUACHandler == null) {
                    waitLocked(TIMES_OUT_MS, "getHandler");
                }
            }
            return mUACHandler;
        }
        ...
        private boolean waitLocked(long timeoutMs, String operation) {
            try {
                mSync.wait(timeoutMs);
                return true;
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                Log.w(TAG, operation + " interrupted", e);
                return false;
            }
        }
    }
