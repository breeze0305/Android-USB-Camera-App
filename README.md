...
為了避免在 GitHub source release 中發布沒有對應 source 的 UAC audio binary，目前乾淨版專案主要聚焦在 USB camera 影像預覽。

設定頁仍保留 camera 聲音播放狀態；如果目前沒有可用的 source-built UAC audio 實作，app 會回報 camera audio 不可用，不會改用手機麥克風作為替代。
