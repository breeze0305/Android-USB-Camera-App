...
class Mp4Muxer(
    ...
    init {
        ...
        try {
            ...
            notifyError("init media muxer failed: ${e.localizedMessage}")
            Logger.e(TAG, "init media muxer failed", e)
    ...
    fun addTracker(mediaFormat: MediaFormat?, isVideo: Boolean) {
        ...
        try {
            ...
            Logger.e(TAG, "addTracker failed", e)
        }
    }
    ...
    fun pumpStream(outputBuffer: ByteBuffer, bufferInfo: MediaCodec.BufferInfo, isVideo: Boolean) {
        try {
            ...
            Logger.e(TAG, "pumpStream failed", e)
        }
    }
    ...
    private fun saveNewFileIfNeed() {
        try {
            ...
            notifyError(e.localizedMessage)
            Logger.e(TAG, "rotate media muxer file failed", e)
        } catch (e: RuntimeException) {
            notifyError(e.localizedMessage)
            Logger.e(TAG, "rotate media muxer file failed", e)
        }
    ...
    fun release() {
        try {
            ...
            notifyError(e.localizedMessage)
            Logger.e(TAG, "release media muxer failed", e)
        } finally {
            mMediaMuxer = null
            mAudioTrackerIndex = -1
            mVideoTrackerIndex = -1
    ...
    private fun getLocalVideoDuration(filePath: String?): Long {
        ...
        return try {
            ...
            Logger.w(TAG, "read local video duration failed", e)
            0L
        } finally {
            try {
                mmr.release()
            } catch (e: RuntimeException) {
                Logger.w(TAG, "release metadata retriever failed", e)
            }
        }
    }
