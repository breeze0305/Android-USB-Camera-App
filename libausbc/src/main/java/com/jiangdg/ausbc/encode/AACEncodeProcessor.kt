...
class AACEncodeProcessor(strategy: IAudioStrategy? = null) : AbstractProcessor(false) {
    ...
    override fun handleStartEncode() {
        ...
        try {
            ...
            releaseCodecAfterStartFailure()
            releaseAudioRecord()
            Logger.e(TAG, "init aac media codec failed", e)
    ...
    override fun handleStopEncode() {
        try {
            ...
            Logger.e(TAG, "release aac media codec failed", e)
        } finally {
            releaseAudioRecord()
            mRawDataQueue.clear()
            mMediaCodec = null
    ...
    fun playAudioStart(callBack: IPlayCallBack?) {
        mAudioThreadPool.submit {
            try {
                ...
                mMainHandler.post {
                    callBack?.onError("Audio playback interrupted")
                }
                Logger.w(TAG, "play mic interrupted", e)
                releaseAudioTrack()
                releaseAudioRecord()
            } catch (e: RuntimeException) {
                mMainHandler.post {
                ...
                Logger.e(TAG, "start/stop play mic failed", e)
                releaseAudioTrack()
                releaseAudioRecord()
            }
        }
     ...
     * Record mp3 start.
     *
     * MP3 recording was removed from this build to avoid shipping the legacy LAME native encoder.
     * @param callBack record status, see [ICaptureCallBack]
     */
    fun recordMp3Start(audioPath: String?, callBack: ICaptureCallBack) {
        val ignoredPath = audioPath?.takeIf { it.isNotBlank() } ?: "default path"
        mMainHandler.post {
            callBack.onError("MP3 recording is not supported in this build; ignored $ignoredPath.")
        }
    }
     ...
     */
    fun recordMp3Stop() {
        // MP3 recording is intentionally disabled.
    }
    ...
    private fun releaseAudioTrack() {
        try {
            ...
            Logger.e(TAG, "releaseAudioTracker failed", e)
        }
    }

    private fun releaseCodecAfterStartFailure() {
        try {
            mMediaCodec?.release()
        } catch (e: RuntimeException) {
            Logger.w(TAG, "release failed aac codec failed", e)
        } finally {
            mMediaCodec = null
        }
    }
