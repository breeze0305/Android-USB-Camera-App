...
class AudioStrategySystem : IAudioStrategy {
    ...
    override fun initAudioRecord() {
        try {
            if (mBufferSize <= 0) {
                Logger.e(TAG, "initAudioRecord failed, invalid min buffer size: $mBufferSize")
                return
            }
            ...
            Logger.e(TAG, "initAudioRecord failed", e)
        }
    }
    ...
    override fun stopRecording() {
        try {
            ...
            Logger.e(TAG, "stopRecording failed", e)
        }
    }
    ...
    override fun releaseAudioRecord() {
        try {
            ...
            Logger.e(TAG, "releaseAudioRecord failed", e)
        }
    }
    ...
    override fun read(): RawData? {
        return if (! isRecording()) {
            ...
            val readBytes = try {
                mAudioRecord?.read(data, 0, mBufferSize) ?: 0
            } catch (e: RuntimeException) {
                Logger.e(TAG, "read audio data failed", e)
                return null
            }
            if (readBytes <= 0) {
                Logger.w(TAG, "read audio data returned $readBytes")
                return null
            }
