...
abstract class AbstractProcessor(private val isVideo: Boolean) {
    ...
    fun setMp4Muxer(muxer: Mp4Muxer) {
        ...
        try {
            ...
            Logger.e(TAG, "set muxer tracker failed, video = $isVideo", e)
    ...
    protected fun doEncodeData() {
        while (isEncoding()) {
            try {
                    ...
                    mMediaCodec?.let { codec ->
                        ...
                        when (outputIndex) {
                            MediaCodec.INFO_OUTPUT_FORMAT_CHANGED -> {
                                ...
                                try {
                                    processOutputBuffer(codec, outputIndex)
                                } finally {
                ...
                Logger.e(TAG, "doEncodeData failed, video = $isVideo", e)
            } catch (e: RuntimeException) {
                Logger.e(TAG, "doEncodeData failed, video = $isVideo", e)
            }
        }
    ...
    private fun queueFrameIfNeed() {
        mMediaCodec?.let { codec ->
            ...
            try {
                ...
                Logger.e(TAG, "queue input buffer overflow, video = $isVideo", e)
            } catch (e: IllegalStateException) {
                Logger.e(TAG, "queue input buffer failed, video = $isVideo", e)
            }
        }
    }

    private fun processOutputBuffer(codec: MediaCodec, outputIndex: Int) {
        try {
            ...
            Logger.e(TAG, "process output buffer failed, video = $isVideo", e)
        } catch (e: RuntimeException) {
            Logger.e(TAG, "process output buffer failed, video = $isVideo", e)
        }
    }
