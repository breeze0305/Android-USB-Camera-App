...
class MultiCameraClient(ctx: Context, callback: IDeviceConnectCallBack?) {
    ...
    abstract class ICamera(val ctx: Context, val device: UsbDevice): Handler.Callback,
                ...
                MSG_START_PREVIEW -> {
                    ...
                    when (val cameraView = mCameraView) {
                        ...
                        val measureSize = try {
                            ...
                            Logger.w(TAG, "surface measure timed out", e)
                            null
        ...
        protected fun releaseEncodeProcessor() {
            releaseMediaMuxer("release encoder")
        ...
        private fun captureVideoStopInternal() {
            ...
            releaseMediaMuxer("stop video capture")
        ...
        private fun releaseMediaMuxer(reason: String) {
            try {
                mMediaMuxer?.release()
            } catch (e: RuntimeException) {
                Logger.e(TAG, "release muxer failed during $reason", e)
            } finally {
                mMediaMuxer = null
            }
        }
