...
class CameraUvcStrategy(ctx: Context) : ICameraStrategy(ctx) {
    ...
    override fun startPreviewInternal() {
        try {
            ...
            stopPreview()
            Logger.e(TAG, " preview interrupted, err = ${e.localizedMessage}", e)
            ...
            stopPreview()
            Logger.e(TAG, " preview cancelled, err = ${e.localizedMessage}", e)
            postCameraStatus(CameraStatus(CameraStatus.ERROR, e.localizedMessage))
        } catch (e: RuntimeException) {
            stopPreview()
            Logger.e(TAG, " preview failed, err = ${e.localizedMessage}", e)
    ...
    private fun realStartPreview(): Boolean? {
        try {
            ...
            Thread.currentThread().interrupt()
            postCameraStatus(CameraStatus(CameraStatus.ERROR, e.localizedMessage))
            Logger.e(TAG, " startPreview interrupted. err = ${e.localizedMessage}", e)
            return null
        } catch (e: ExecutionException) {
            postCameraStatus(CameraStatus(CameraStatus.ERROR, e.localizedMessage))
            ...
            postCameraStatus(CameraStatus(CameraStatus.ERROR, e.localizedMessage))
            Logger.e(TAG, " startPreview cancelled. err = ${e.localizedMessage}", e)
            return null
        } catch (e: RuntimeException) {
            postCameraStatus(CameraStatus(CameraStatus.ERROR, e.localizedMessage))
            Logger.e(TAG, " startPreview failed. err = ${e.localizedMessage}", e)
    ...
    fun getCurrentDevice(): UsbDevice? {
        return try {
            ...
            Logger.w(TAG, "get current device failed: ${e.localizedMessage}")
            null
        } catch (e: CancellationException) {
            Logger.w(TAG, "get current device cancelled: ${e.localizedMessage}")
            null
        }
    }
    ...
    fun getUsbControlBlock(): USBMonitor.UsbControlBlock? {
        return try {
            ...
            Thread.currentThread().interrupt()
            Logger.w(TAG, "get current usb control block interrupted")
            null
        } catch (e: ExecutionException) {
            Logger.w(TAG, "get current usb control block failed: ${e.localizedMessage}")
            null
        } catch (e: CancellationException) {
            Logger.w(TAG, "get current usb control block times out")
            null
