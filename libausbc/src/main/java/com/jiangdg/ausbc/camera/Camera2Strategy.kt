...
class Camera2Strategy(ctx: Context) : ICameraStrategy(ctx) {
    ...
    override fun captureImageInternal(savePath: String?) {
        ...
        try {
            ...
            Logger.e(TAG, "takePictureInternal failed", e)
        }
    ...
    private fun createSession() {
        try {
            ...
            Logger.e(TAG, "createCaptureSession failed, err = ${e.reason}", e)
        } catch (e: RuntimeException) {
            Logger.e(TAG, "createCaptureSession failed", e)
        }
    }
    ...
    private val mPreviewAvailableListener = ImageReader.OnImageAvailableListener { imageReader ->
        ...
        image?.use {
            ...
            try {
                ...
                Logger.e(TAG, "preview image conversion failed", e)
            }
        }
    }
        ...
        image?.use {
            ...
            mSaveImageExecutor.submit {
                ...
                try {
                    ...
                    Logger.e(TAG, "image save path was cancelled", e)
                    mMainHandler.post {
                        mCaptureDataCb?.onError("set path failed, save auto ")
                    }
                } catch (e: ExecutionException) {
                    Logger.e(TAG, "image save path failed", e)
                    mMainHandler.post {
                        mCaptureDataCb?.onError("set path failed, save auto ")
                    }
                } catch (e: InterruptedException) {
                    Thread.currentThread().interrupt()
                    Logger.e(TAG, "image save path interrupted", e)
                    mMainHandler.post {
                        mCaptureDataCb?.onError("set path failed, save auto ")
                    }
                }
