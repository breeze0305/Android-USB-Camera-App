...
object CrashUtils : Thread.UncaughtExceptionHandler {
    ...
    override fun uncaughtException(t: Thread, e: Throwable) {
        if (!handleException(e) && mDefaultHandler != null) {
            ...
            try {
                ...
                Logger.w(TAG, "crash restart delay interrupted", e1)
            }
            restartAppIfNeed()
        }
    }
    ...
    private fun saveErrorInfo(ex: Throwable): File? {
        ...
        try {
            ...
            Logger.e(TAG, "save crash info failed", e1)
        } catch (e: PackageManager.NameNotFoundException) {
            Logger.e(TAG, "read package info for crash report failed", e)
        } finally {
            try {
                printWriter?.close()
                fw?.close()
            } catch (e: IOException) {
                Logger.e(TAG, "close crash report writer failed", e)
            }
        }
        return logFile
    }
