...
class MainActivity : CameraActivity() {
        ...
        runOnUiThread {
            when (code) {
                ICameraStateCallBack.State.OPENED -> {
                    ...
                    statusText.text = "已連線：${CameraSettingsStore.labelForDevice(self.getUsbDevice())} " +
                        "${size?.width ?: settings.width}x${size?.height ?: settings.height}"
                    applyPreviewTransform()
                }
    ...
    private fun applySettings(newSettings: CameraSettings) {
        ...
        when {
            needsReopen -> reopenCameraAfterSettingsChange()
            needsResolutionUpdate -> updateResolution(newSettings.width, newSettings.height)
        }
    }

    private fun reopenCameraAfterSettingsChange() {
    ...
    private fun updateDisplayModeButtonIcon() {
        ...
        val label = when (displayMode) {
            DisplayMode.CONTAIN -> "完整顯示"
            DisplayMode.STRETCH -> "拉伸填滿"
            DisplayMode.COVER -> "等比覆蓋"
        }
